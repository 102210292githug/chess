...
<script type="text/javascript">
    ...
    const Type = "<c:out value ='${Type}'/>";
    const user = JSON.parse('${userJson}');
    const user_oth = JSON.parse('${userOthJson}');
    console.log(Type);
    console.log(user);
    console.log(user_oth);
    
    document.getElementById('username_top').textContent = user_oth.username;
    document.getElementById('elo_top').textContent = 'Elo: ' + user_oth.elo;

    // Cập nhật thông tin người chơi dưới
	...
	*/
	const userID = '${sessionScope.userID}'; console.log(userID);
    const Type = "<c:out value ='${Type}'/>";console.log(Type);
	window.onload = function() {
        const user = JSON.parse('${userJson}');
        const user_oth = JSON.parse('${userOthJson}');
        console.log(user);
